...
int pf_trajectory_create(TrajectoryInfo info, TrajectoryConfig c, Segment *seg) {
    int ret = pf_trajectory_fromSecondOrderFilter(info.filter1, info.filter2, info.dt, info.u, info.v, info.impulse, info.length, seg, c);
...
// Modified Method by @shaylandias to generate the headings at the same time as all other Segment values, allowing them to be streamed by NetworkTables immediately
int pf_trajectory_fromSecondOrderFilter(int filter_1_l, int filter_2_l, double dt, double u, double v, double impulse, 
        int len, Segment *t, TrajectoryConfig c) {
    Segment last_section = {dt, 0, 0, 0, u, 0, 0};
    
    // NetworkTableInstance nt = NetworkTableInstance.GetDefault();
    // NetworkTable table = nt.GetEntry();
    ...
    for (i = 0; i < len; i++) {
        ...
        double d_theta = c.dest_theta - c.src_theta;
        int i;
        t[i].heading = c.src_theta + d_theta * (t[i].position) / (t[len - 1].position);
